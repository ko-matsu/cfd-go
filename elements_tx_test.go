package cfdgo

import (
	"fmt"
	"testing"

	"github.com/stretchr/testify/assert"
)

func TestCreateClaimPeginTx(t *testing.T) {
	network := ElementsRegtest
	genesisBlockHash := "0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206"
	asset := "5ac9f65c0efcc4775e0baec4ec03abdde22473cd3cf33c0419ca290e0751b225"

	// fedpeg script
	fedpegScript := "522103aab896d53a8e7d6433137bbba940f9c521e085dd07e60994579b64a6d992cf79210291b7d0b1b692f8f524516ed950872e5da10fb1b808b5a526dedc6fed1cf29807210386aa9372fbab374593466bc5451dc59954e90787f08060964d95c87ef34ca5bb53ae"
	privkey, err := NewPrivkeyFromWif("cUfipPioYnHU61pfYTH9uuNoswRXx8rtzXhJZrsPeVV1LRFdTxvp")
	assert.NoError(t, err)
	pubkey, err := privkey.GetPubkey()
	assert.NoError(t, err)

	// create pegin address
	addrUtil := AddressUtil{Network: &network}
	peginAddr, claimScript, err := addrUtil.GetPeginAddressByPubkey(P2shP2wshAddress, fedpegScript, pubkey.Hex)
	assert.NoError(t, err)
	assert.Equal(t, "2MvmzAFKZ5xh44vyb7qY7NB2AoDuS55rVFW", peginAddr.Address)
	assert.Equal(t, "0014e794713e386d83f32baa0e9d03e47c0839dc57a8", claimScript.ToHex())

	// create bitcoin tx
	amount := int64(100000000)
	feeAmount := int64(500)
	peginAmount := amount - feeAmount
	btcNetwork := network.ToBitcoinType()
	btcTxUtil := TransactionUtil{Network: &btcNetwork}
	utxoOutPoint := OutPoint{
		Txid: "ea9d5a9e974af1d167305aa6ee598706d63274e8a40f4f33af97db37a7adde4c",
		Vout: 0,
	}
	btcInputs := []InputTxIn{
		{
			OutPoint: utxoOutPoint,
			Sequence: SequenceLockTimeFinal,
		},
	}
	btcOutputs := []InputTxOut{
		{
			Amount:  peginAmount,
			Address: peginAddr.Address,
		},
	}
	btcTx, err := btcTxUtil.Create(uint32(2), uint32(0), &btcInputs, &btcOutputs)
	assert.NoError(t, err)
	// add sign
	utxos := []UtxoData{
		{
			Txid:       utxoOutPoint.Txid,
			Vout:       utxoOutPoint.Vout,
			Amount:     amount,
			Descriptor: "wpkh(02fd54c734e48c544c3c3ad1aab0607f896eb95e23e7058b174a580826a7940ad8)",
		},
	}
	utxoPrivkey, err := NewPrivkeyFromWif("cNYKHjNc33ZyNMcDck59yWm1CYohgPhr2DYyCtmWNkL6sqb5L1rH")
	assert.NoError(t, err)
	err = btcTx.SignWithPrivkey(&utxoOutPoint, utxoPrivkey, SigHashTypeAll, &utxos)
	assert.NoError(t, err)
	assert.Equal(t, "", btcTx.Hex)

	assert.Equal(t, "12708508f0baf8691a3d7e22fd19afbf9bd8bf0d358e3310838bcc7916539c7b", btcTx.GetTxid())

	peginIndex := uint32(0)
	txoutProof := ""

	// create pegin tx
	txUtil := ElementsTransactionUtil{Network: &network}
	peginOutPoint := OutPoint{
		Txid: btcTx.GetTxid(),
		Vout: peginIndex,
	}
	inputs := []InputConfidentialTxIn{
		{
			OutPoint: peginOutPoint,
			PeginInput: &InputPeginData{
				BitcoinTransaction:      btcTx.Hex,
				BitcoinGenesisBlockHash: genesisBlockHash,
				BitcoinAssetId:          asset,
				ClaimScript:             claimScript.ToHex(),
				TxOutProof:              txoutProof,
			},
		},
	}
	outputs := []InputConfidentialTxOut{
		{
			Amount:  99998500,
			Address: "el1qqtl9a3n6878ex25u0wv8u5qlzpfkycc0cftk65t52pkauk55jqka0fajk8d80lafn4t9kqxe77cu9ez2dyr6sq54lwy009uex",
			Asset:   asset,
		},
		{
			Amount: 1000,
			Asset:  asset,
			IsFee:  true,
		},
	}
	tx, err := txUtil.Create(uint32(2), uint32(0), &inputs, &outputs, nil)
	assert.NoError(t, err)
	assert.Equal(t, "", tx.Hex)

	// blind
	blindTxInList := []BlindInputData{
		{
			OutPoint: peginOutPoint,
			Asset:    asset,
			Amount:   peginAmount,
		},
	}
	option := NewBlindTxOption()
	option.AppendDummyOutput = true
	err = tx.Blind(blindTxInList, nil, &option)
	assert.NoError(t, err)
	_, inList, outList, err := tx.GetAll(false)
	assert.NoError(t, err)
	assert.Equal(t, 1, len(inList))
	assert.Equal(t, 3, len(outList))

	// sign
	peginUtxos := []UtxoData{
		{
			Txid:       peginOutPoint.Txid,
			Vout:       peginOutPoint.Vout,
			Amount:     peginAmount,
			Descriptor: "wpkh(" + pubkey.Hex + ")",
		},
	}
	sighash, err := tx.GetSighash(&peginOutPoint, SigHashTypeAll, &peginUtxos)
	assert.NoError(t, err)
	descriptor := NewDescriptorFromString(peginUtxos[0].Descriptor, network.ToCfdValue())
	signature, err := privkey.CreateEcSignature(sighash, &SigHashTypeAll)
	assert.NoError(t, err)

	// add sign
	err = tx.AddPubkeySignByDescriptor(&peginOutPoint, descriptor, signature.ToHex())
	assert.NoError(t, err)

	// verify
	isVerify, reason, err := tx.VerifySign(&peginOutPoint, peginAmount, &peginUtxos)
	assert.NoError(t, err)
	assert.True(t, isVerify)
	assert.Equal(t, "", reason)

	fmt.Printf("%s test done.\n", GetFuncName())
}

func TestCreatePegoutTx(t *testing.T) {
	network := LiquidV1
	genesisBlockHash := "0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206"
	asset := "5ac9f65c0efcc4775e0baec4ec03abdde22473cd3cf33c0419ca290e0751b225"

	// mainchain address descriptor
	mainchainXpubkey := "xpub6A53gzNdDBQYCtFFpZT7kUpoBGpzWigaukrdF9xoUZt7cYMD2qCAHVLsstNoQEDMFJWdX78KvT6yxpC76aGCN5mENVdWtFGcWZoKdtLq5jW"
	mainchainPubkey, err := CfdGoGetPubkeyFromExtkey(mainchainXpubkey, int(KCfdNetworkMainnet))
	assert.NoError(t, err)
	negateMainchainPubkey, err := CfdGoNegatePubkey(mainchainPubkey)
	assert.NoError(t, err)
	mainchainOutputDescriptor := "pkh(" + mainchainXpubkey + "/0/*)"
	bip32Counter := uint32(0)

	onlinePrivkey := "L52AgshDAE14NHJuovwAw8hyrTNK4YQjuiPC9EES4sfM7oBPzU4o"
	onlinePubkey, err := CfdGoGetPubkeyFromPrivkey("", onlinePrivkey, true)
	assert.NoError(t, err)
	// whitelist
	pakEntry := negateMainchainPubkey + onlinePubkey
	whitelist := pakEntry

	// pegout address
	addrUtil := AddressUtil{Network: &network}
	pegoutAddr, baseDescriptor, err := addrUtil.GetPegoutAddress(P2pkhAddress, mainchainOutputDescriptor, bip32Counter)
	assert.NoError(t, err)
	assert.Equal(t, "1NrcpiZmCxjC7KVKAYT22SzVhhcXtp5o4v", pegoutAddr.Address)
	assert.Equal(t, "pkh("+mainchainXpubkey+")", *baseDescriptor)

	// create pegout tx
	txUtil := ElementsTransactionUtil{Network: &network}
	pegoutAddrList := []string{}
	inputs := []InputConfidentialTxIn{
		{
			OutPoint: OutPoint{
				Txid: "4aa201f333e80b8f62ba5b593edb47b4730212e2917b21279f389ba1c14588a3",
				Vout: 0,
			},
			Sequence: 4294967293,
		},
	}
	outputs := []InputConfidentialTxOut{
		{
			Amount:  209998999992700,
			Address: "XBMr6srTXmWuHifFd8gs54xYfiCBsvrksA",
			Asset:   asset,
		},
		{
			Amount: 1000000000,
			Asset:  asset,
			PegoutInput: &InputPegoutData{
				OnlineKey:               onlinePrivkey,
				BitcoinGenesisBlockHash: genesisBlockHash,
				BitcoinOutputDescriptor: mainchainOutputDescriptor,
				Bip32Counter:            bip32Counter,
				Whitelist:               whitelist,
			},
		},
		{
			Amount: 7300,
			Asset:  asset,
			IsFee:  true,
		},
	}
	tx, err := txUtil.Create(uint32(2), uint32(0), &inputs, &outputs, &pegoutAddrList)
	assert.NoError(t, err)
	assert.Equal(t, "", tx.Hex)
	assert.Equal(t, 1, len(pegoutAddrList))
	assert.Equal(t, "1NrcpiZmCxjC7KVKAYT22SzVhhcXtp5o4v", pegoutAddrList[0])

	pegoutAddress, hasPegout, err := tx.GetPegoutAddress(uint32(1))
	assert.NoError(t, err)
	assert.True(t, hasPegout)
	assert.Equal(t, "1NrcpiZmCxjC7KVKAYT22SzVhhcXtp5o4v", pegoutAddress.Address)

	fmt.Printf("%s test done.\n", GetFuncName())
}

func TestCreateClaimPeginTxByCfdConf(t *testing.T) {
	SetCfdConfig(CfdConfig{
		Network:                 ElementsRegtest,
		BitcoinGenesisBlockHash: "0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206",
		BitcoinAssetId:          "5ac9f65c0efcc4775e0baec4ec03abdde22473cd3cf33c0419ca290e0751b225",
	})

	// fedpeg script
	fedpegScript := "522103aab896d53a8e7d6433137bbba940f9c521e085dd07e60994579b64a6d992cf79210291b7d0b1b692f8f524516ed950872e5da10fb1b808b5a526dedc6fed1cf29807210386aa9372fbab374593466bc5451dc59954e90787f08060964d95c87ef34ca5bb53ae"
	privkey, err := NewPrivkeyFromWif("cUfipPioYnHU61pfYTH9uuNoswRXx8rtzXhJZrsPeVV1LRFdTxvp")
	assert.NoError(t, err)
	pubkey, err := privkey.GetPubkey()
	assert.NoError(t, err)

	// create pegin address
	addrUtil := AddressUtil{}
	peginAddr, claimScript, err := addrUtil.GetPeginAddressByPubkey(P2shP2wshAddress, fedpegScript, pubkey.Hex)
	assert.NoError(t, err)
	assert.Equal(t, "2MvmzAFKZ5xh44vyb7qY7NB2AoDuS55rVFW", peginAddr.Address)
	assert.Equal(t, "0014e794713e386d83f32baa0e9d03e47c0839dc57a8", claimScript.ToHex())

	// create bitcoin tx
	amount := int64(100000000)
	feeAmount := int64(500)
	peginAmount := amount - feeAmount
	btcTxUtil := TransactionUtil{}
	utxoOutPoint := OutPoint{
		Txid: "ea9d5a9e974af1d167305aa6ee598706d63274e8a40f4f33af97db37a7adde4c",
		Vout: 0,
	}
	btcInputs := []InputTxIn{
		{
			OutPoint: utxoOutPoint,
			Sequence: SequenceLockTimeFinal,
		},
	}
	btcOutputs := []InputTxOut{
		{
			Amount:  peginAmount,
			Address: peginAddr.Address,
		},
	}
	btcTx, err := btcTxUtil.Create(uint32(2), uint32(0), &btcInputs, &btcOutputs)
	assert.NoError(t, err)
	// add sign
	utxos := []UtxoData{
		{
			Txid:       utxoOutPoint.Txid,
			Vout:       utxoOutPoint.Vout,
			Amount:     amount,
			Descriptor: "wpkh(02fd54c734e48c544c3c3ad1aab0607f896eb95e23e7058b174a580826a7940ad8)",
		},
	}
	utxoPrivkey, err := NewPrivkeyFromWif("cNYKHjNc33ZyNMcDck59yWm1CYohgPhr2DYyCtmWNkL6sqb5L1rH")
	assert.NoError(t, err)
	err = btcTx.SignWithPrivkey(&utxoOutPoint, utxoPrivkey, SigHashTypeAll, &utxos)
	assert.NoError(t, err)
	assert.Equal(t, "", btcTx.Hex)

	assert.Equal(t, "12708508f0baf8691a3d7e22fd19afbf9bd8bf0d358e3310838bcc7916539c7b", btcTx.GetTxid())

	peginIndex := uint32(0)
	txoutProof := ""

	// create pegin tx
	txUtil := ElementsTransactionUtil{}
	peginOutPoint := OutPoint{
		Txid: btcTx.GetTxid(),
		Vout: peginIndex,
	}
	inputs := []InputConfidentialTxIn{
		{
			OutPoint: peginOutPoint,
			PeginInput: &InputPeginData{
				BitcoinTransaction: btcTx.Hex,
				ClaimScript:        claimScript.ToHex(),
				TxOutProof:         txoutProof,
			},
		},
	}
	outputs := []InputConfidentialTxOut{
		{
			Amount:  99998500,
			Address: "el1qqtl9a3n6878ex25u0wv8u5qlzpfkycc0cftk65t52pkauk55jqka0fajk8d80lafn4t9kqxe77cu9ez2dyr6sq54lwy009uex",
		},
		{
			Amount: 1000,
			IsFee:  true,
		},
	}
	tx, err := txUtil.Create(uint32(2), uint32(0), &inputs, &outputs, nil)
	assert.NoError(t, err)
	assert.Equal(t, "", tx.Hex)

	// blind
	blindTxInList := []BlindInputData{
		{
			OutPoint: peginOutPoint,
			Amount:   peginAmount,
		},
	}
	option := NewBlindTxOption()
	option.AppendDummyOutput = true
	err = tx.Blind(blindTxInList, nil, &option)
	assert.NoError(t, err)
	_, inList, outList, err := tx.GetAll(false)
	assert.NoError(t, err)
	assert.Equal(t, 1, len(inList))
	assert.Equal(t, 3, len(outList))

	// create signature
	peginUtxos := []UtxoData{
		{
			Txid:       peginOutPoint.Txid,
			Vout:       peginOutPoint.Vout,
			Amount:     peginAmount,
			Descriptor: "wpkh(" + pubkey.Hex + ")",
		},
	}
	sighash, err := tx.GetSighash(&peginOutPoint, SigHashTypeAll, &peginUtxos)
	assert.NoError(t, err)
	descriptor := &Descriptor{OutputDescriptor: peginUtxos[0].Descriptor}
	signature, err := privkey.CreateEcSignature(sighash, &SigHashTypeAll)
	assert.NoError(t, err)

	// add sign
	err = tx.AddPubkeySignByDescriptor(&peginOutPoint, descriptor, signature.ToHex())
	assert.NoError(t, err)

	// verify
	isVerify, reason, err := tx.VerifySign(&peginOutPoint, peginAmount, &peginUtxos)
	assert.NoError(t, err)
	assert.True(t, isVerify)
	assert.Equal(t, "", reason)

	fmt.Printf("%s test done.\n", GetFuncName())
}

func TestCreatePegoutTxByCfdConf(t *testing.T) {
	SetCfdConfig(CfdConfig{
		Network:                 NewNetworkTypeByString("liquidv1"),
		BitcoinGenesisBlockHash: "0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206",
		BitcoinAssetId:          "5ac9f65c0efcc4775e0baec4ec03abdde22473cd3cf33c0419ca290e0751b225",
	})

	// mainchain address descriptor
	mainchainXpubkey := "xpub6A53gzNdDBQYCtFFpZT7kUpoBGpzWigaukrdF9xoUZt7cYMD2qCAHVLsstNoQEDMFJWdX78KvT6yxpC76aGCN5mENVdWtFGcWZoKdtLq5jW"
	mainchainPubkey, err := CfdGoGetPubkeyFromExtkey(mainchainXpubkey, int(KCfdNetworkMainnet))
	assert.NoError(t, err)
	negateMainchainPubkey, err := CfdGoNegatePubkey(mainchainPubkey)
	assert.NoError(t, err)
	mainchainOutputDescriptor := "pkh(" + mainchainXpubkey + "/0/*)"
	bip32Counter := uint32(0)

	onlinePrivkey := "L52AgshDAE14NHJuovwAw8hyrTNK4YQjuiPC9EES4sfM7oBPzU4o"
	onlinePubkey, err := CfdGoGetPubkeyFromPrivkey("", onlinePrivkey, true)
	assert.NoError(t, err)
	// whitelist
	pakEntry := negateMainchainPubkey + onlinePubkey
	whitelist := pakEntry

	// pegout address
	addrUtil := AddressUtil{}
	pegoutAddr, baseDescriptor, err := addrUtil.GetPegoutAddress(P2pkhAddress, mainchainOutputDescriptor, bip32Counter)
	assert.NoError(t, err)
	assert.Equal(t, "1NrcpiZmCxjC7KVKAYT22SzVhhcXtp5o4v", pegoutAddr.Address)
	assert.Equal(t, "pkh("+mainchainXpubkey+")", *baseDescriptor)

	// create pegout tx
	txUtil := ElementsTransactionUtil{}
	pegoutAddrList := []string{}
	inputs := []InputConfidentialTxIn{
		{
			OutPoint: OutPoint{
				Txid: "4aa201f333e80b8f62ba5b593edb47b4730212e2917b21279f389ba1c14588a3",
				Vout: 0,
			},
			Sequence: 4294967293,
		},
	}
	outputs := []InputConfidentialTxOut{
		{
			Amount:  209998999992700,
			Address: "XBMr6srTXmWuHifFd8gs54xYfiCBsvrksA",
		},
		{
			Amount: 1000000000,
			PegoutInput: &InputPegoutData{
				OnlineKey:               onlinePrivkey,
				BitcoinOutputDescriptor: mainchainOutputDescriptor,
				Bip32Counter:            bip32Counter,
				Whitelist:               whitelist,
			},
		},
		{
			Amount: 7300,
			IsFee:  true,
		},
	}
	tx, err := txUtil.Create(uint32(2), uint32(0), &inputs, &outputs, &pegoutAddrList)
	assert.NoError(t, err)
	assert.Equal(t, "", tx.Hex)
	assert.Equal(t, 1, len(pegoutAddrList))
	assert.Equal(t, "1NrcpiZmCxjC7KVKAYT22SzVhhcXtp5o4v", pegoutAddrList[0])

	pegoutAddress, hasPegout, err := tx.GetPegoutAddress(uint32(1))
	assert.NoError(t, err)
	assert.True(t, hasPegout)
	assert.Equal(t, "1NrcpiZmCxjC7KVKAYT22SzVhhcXtp5o4v", pegoutAddress.Address)

	fmt.Printf("%s test done.\n", GetFuncName())
}
