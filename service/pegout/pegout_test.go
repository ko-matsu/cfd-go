...
func TestCreatePegoutTxMaximumWhitelist(t *testing.T) {
	...
	assert.Equal(t, 1, len(inList))
	assert.Equal(t, 3, len(outList)) // pegout, fee, output(change)
	assert.Less(t, 23110, len(tx.Hex))
	assert.Greater(t, 23120, len(tx.Hex))
	_, _, unblindTxoutList, err := txApi.GetAll(unblindTx, false)
	assert.NoError(t, err)
	assert.Equal(t, int64(1000000000), unblindTxoutList[0].Amount)
	assert.Equal(t, int64(1404), unblindTxoutList[1].Amount)
	assert.Equal(t, int64(18999998596), unblindTxoutList[2].Amount)
