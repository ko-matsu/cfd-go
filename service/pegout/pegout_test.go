...
func TestCreatePegoutTxSegwit(t *testing.T) {
	config.SetCfdConfig(config.CfdConfig{
		Network:                 types.NewNetworkTypeByString("liquidv1"),
		BitcoinGenesisBlockHash: "0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206",
		BitcoinAssetId:          "5ac9f65c0efcc4775e0baec4ec03abdde22473cd3cf33c0419ca290e0751b225",
	...
	// pegout address
	// pegoutAddr, desc, err := pegoutApi.CreatePegoutAddress(types.P2wpkhAddress, &mainchainXpubkey, addressIndex)
	// assert.NoError(t, err)
	// assert.Equal(t, "1D4YiPF4k9qotSS3QWMa2E8Bt4jV9SZPmE", pegoutAddr.Address)
	// assert.Equal(t, "wpkh(xpub6DEq98J615HL2A5UXP5DVPmEtet7DXAsqQHEBvfbEcwAC9PBKu9cG3tCkU5fXkiaJkeQzc81YiY6DDUg82eGx2dr7NpvBXstZvw5M6wisVo)", desc.OutputDescriptor)
	desc := "wpkh(xpub6DEq98J615HL2A5UXP5DVPmEtet7DXAsqQHEBvfbEcwAC9PBKu9cG3tCkU5fXkiaJkeQzc81YiY6DDUg82eGx2dr7NpvBXstZvw5M6wisVo)"
	...
	tx, pegoutAddr, unblindTx, err := pegoutApi.CreatePegoutTransaction(utxos, pegoutData, nil, &changeAddress, &option)
	assert.NoError(t, err)
	assert.Equal(t, "bc1qs3xn47eqs80kslnvp2pa9f0qjfwylgqlec9dn0", pegoutAddr.Address)
...
func TestCreatePegoutTxMaximumWhitelist(t *testing.T) {
	config.SetCfdConfig(config.CfdConfig{
		Network:                 types.NewNetworkTypeByString("liquidv1"),
		BitcoinGenesisBlockHash: "0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206",
		BitcoinAssetId:          "5ac9f65c0efcc4775e0baec4ec03abdde22473cd3cf33c0419ca290e0751b225",
	...
	assert.NoError(t, err)
	whitelist := ""
	// pks := make([]string, 255) // maximum over. (current: pass by zkp's bug)
	pks := make([]string, 254)
	for i := range pks {
		pk1, _, _, err := cfdgo.CfdGoCreateKeyPair(true, types.Mainnet.ToCfdValue())
		assert.NoError(t, err)
		pk2, _, _, err := cfdgo.CfdGoCreateKeyPair(true, types.Mainnet.ToCfdValue())
		assert.NoError(t, err)
		pks[i] = pk1 + pk2
		whitelist += pks[i]
	}
	...
	assert.Equal(t, 1, len(inList))
	assert.Equal(t, 3, len(outList)) // pegout, fee, output(change)
	assert.Less(t, 23040, len(tx.Hex))
	assert.Greater(t, 23060, len(tx.Hex))
	_, _, unblindTxoutList, err := txApi.GetAll(unblindTx, false)
	assert.NoError(t, err)
	assert.Equal(t, int64(1000000000), unblindTxoutList[0].Amount)
	assert.Equal(t, int64(1399), unblindTxoutList[1].Amount)
	assert.Equal(t, int64(18999998601), unblindTxoutList[2].Amount)
